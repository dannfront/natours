"use strict";

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function _createClass(Constructor, protoProps, staticProps) { if (protoProps) _defineProperties(Constructor.prototype, protoProps); if (staticProps) _defineProperties(Constructor, staticProps); return Constructor; }

// bblioteca para enviar correos
var nodeMailer = require('nodemailer');

var pug = require('pug');

var htmlToText = require('html-to-text'); // clase para enviar email


module.exports =
/*#__PURE__*/
function () {
  // el cosntructor solo toma dos parametros el usuario y la url
  function Email(user, url) {
    _classCallCheck(this, Email);

    // toma el email del usuario porque se le pasa el objeto del usuario
    this.to = user.email;
    this.firstName = user.name.split(" ")[0];
    this.url = url;
    this.from = "Santiago Daniel <".concat(process.env.EMAIL_FROM, ">");
  }

  _createClass(Email, [{
    key: "transport",
    value: function transport() {
      if (process.env.NODE_ENV === "production") {
        return nodeMailer.createTransport({
          host: process.env.BREVO_HOST,
          port: process.env.BREVO_PORT,
          auth: {
            user: process.env.BREVO_USERNAME,
            pass: process.env.BREVO_PASSWORD
          }
        });
      }

      return nodeMailer.createTransport({
        //servicio que usaremos para enviar correos
        host: process.env.EMAIL_HOST,
        port: process.env.EMAIL_PORT,
        //autenticacion: desde que dirrecion de correo enviaremos los correos
        auth: {
          user: process.env.EMAIL_USERNAME,
          pass: process.env.EMAIL_PASSWORD
        } // 2 definimos las opciones del email

      });
    }
  }, {
    key: "send",
    value: function send(template, subject) {
      var html, optiosEmail;
      return regeneratorRuntime.async(function send$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              //1 renderizar la plantilla pug
              html = pug.renderFile("".concat(__dirname, "/../views/emails/").concat(template, ".pug"), {
                // varibles que ocuoaremos en la plantilla
                firstName: this.firstName,
                url: this.url,
                subject: subject
              }); // definir las opcines de email

              optiosEmail = {
                // desde donde enviaremos el gmail
                from: this.from,
                //datos de adonde se enviara el coreeo
                to: this.to,
                subject: subject,
                // manadamos la plantilla html
                html: html,
                text: htmlToText.convert(html)
              }; // enviamos el email, sendMail es del paquete nodeMailer no tiene que ver con la clase

              _context.next = 4;
              return regeneratorRuntime.awrap(this.transport().sendMail(optiosEmail));

            case 4:
            case "end":
              return _context.stop();
          }
        }
      }, null, this);
    }
  }, {
    key: "sendWelcome",
    value: function sendWelcome() {
      return regeneratorRuntime.async(function sendWelcome$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              _context2.next = 2;
              return regeneratorRuntime.awrap(this.send("welcome", "welcome to the Natours Family "));

            case 2:
            case "end":
              return _context2.stop();
          }
        }
      }, null, this);
    }
  }, {
    key: "passwordReset",
    value: function passwordReset() {
      return regeneratorRuntime.async(function passwordReset$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              _context3.next = 2;
              return regeneratorRuntime.awrap(this.send("passwordReset", "Your password reset (valid for only 10 minutes)"));

            case 2:
            case "end":
              return _context3.stop();
          }
        }
      }, null, this);
    }
  }]);

  return Email;
}();