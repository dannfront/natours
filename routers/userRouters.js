...
const userHanddler=require('./../controller/userController.js')
...
route.get('/logout',authController.loggingOut)
// route.post('/conmfirmEmail/:token',authController.conmfirmEmail)
...
//obtener los datos del usaurio sin que ponga el id en el url
route.get("/me",userController.me,userController.showUser)

route.use(authController.restricted("admin"))

route.route('/')
    .get(userHanddler.getAllUsers)
    .post(userHanddler.createUser)

route.route('/:id')
    .get(userHanddler.showUser)
    .patch(authController.restricted('admin'),userHanddler.updateUser)
    .delete(authController.restricted("admin"),userHanddler.deleteUser)
